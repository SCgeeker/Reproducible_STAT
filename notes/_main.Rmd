...
demo_place <- "花蓮"
demo_cards <- c("龍","虎","狼")
demo_pick <- sample(demo_cards, 1)
demo_numbers <- c(10000,5000,1000)
```

我的名字是```r demo_name```，現在住在```r demo_place```。袋子裡有三張卡片，印著```r demo_cards```，今天早上抽出的卡片是```r demo_pick```，價值是```r demo_numbers[which(demo_cards == demo_pick)]```。
...
<!--chapter:end:objects_practice.Rmd-->
